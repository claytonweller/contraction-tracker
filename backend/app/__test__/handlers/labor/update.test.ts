import { updateHandler as updateLaborHandler } from "../../../handlers/labor/update";
...
describe('updateLaborHandler', ()=>{
  const dynamoSpy = jest.spyOn(dynamo.labor, 'updateActive')
  ...
  describe('when the body is incorrectly formatted', ()=>{
    ...
    beforeEach(async ()=>{
      try {  
        await updateLaborHandler(incorrect)
      } catch (e){
        error = e
      }
    })  
  ...
  describe('when the body is correctly formatted', ()=>{
    ...
    describe('and the labor is successfully persisted', ()=>{
      ...
      beforeEach(async()=>{
        ...
        result = await updateLaborHandler(correct)
      })
    ...
    describe('and the labor is NOT successfully persisted', ()=>{
      ...
      beforeEach(async ()=>{
        ...
        try {  
          await updateLaborHandler(correct)
        } catch (e){
          error = e
        }
      })  
